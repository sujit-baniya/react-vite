...
import {IAxiosConfig, IAxiosQueueManager, IAxiosQueueManagerProps, IHttpMethod, IRequestData} from './interfaces';
...
class AxiosQueueManager implements IAxiosQueueManager {
    ...
    request(url: string, method: IHttpMethod, data?: any, config?: IAxiosConfig): Promise<AxiosResponse<any>> {
        return new Promise<AxiosResponse<any>>((resolve, reject) => {
            this.createTask({url, data, config, method, onResolve: resolve, onReject: reject});
        });
    }
